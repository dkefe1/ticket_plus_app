...
class UpcomingScreen extends StatefulWidget {
  ...
  @override
  State<UpcomingScreen> createState() => _UpcomingScreenStateScreen();
}

class _UpcomingScreenStateScreen extends State<UpcomingScreen> {
  final pref = PrefService();
  bool isUpcomingEmpty = false;

  @override
  void initState() {
    BlocProvider.of<EventBloc>(context).add(GetUpcomingEventListEvent());
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myAppBar(context, "Upcoming"),
      body: BlocConsumer<EventBloc, EventState>(
        listener: (context, state) {},
        builder: (context, state) {
          if (state is UpcomingEventLoadingState) {
            return buildLoading();
          } else if (state is UpcomingEventSuccessfulState) {
            isUpcomingEmpty = state.event.isEmpty;
  ...
  Widget buildInitialInput({required List<Event> event}) {
    ...
    return isUpcomingEmpty
        ? Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                Text("There is no upcoming Event this week"),
              ],
            ),
          )
        : Container(
