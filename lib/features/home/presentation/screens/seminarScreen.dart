...
class _SeminarScreenState extends State<SeminarScreen> {
      ...
      body: BlocConsumer<EventBloc, EventState>(
        ...
        builder: (context, state) {
          if (state is EventLoadingState) {
            ...
            isSeminarEmpty = state.event.isEmpty;
            var seminarEvents = state.event
                .where((event) => event.category[0].category_name == "Sport")
                .toList();
            return buildInitialInput(event: seminarEvents);
          } else {
  ...
  Widget buildInitialInput({required List<Event> event}) {
    ...
    return isSeminarEmpty
        ? Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                Text("There are no Seminars"),
              ],
            ),
          )
        : Container(
