...
class _SportScreenState extends State<SportScreen> {
  ...
  bool isSportEmpty = false;
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myAppBar(context, "Sports"),
      body: BlocConsumer<EventBloc, EventState>(
        ...
        builder: (context, state) {
          if (state is EventLoadingState) {
            ...
            isSportEmpty = state.event.isEmpty;
            var sportEvents = state.event
                .where((event) => event.category[0].category_name == "Sport")
                .toList();
            return buildInitialInput(event: sportEvents);
          } else {
    ...
    return isSportEmpty
        ? Center(
            child: Column(
              ...
              children: [
                Text("There are no Sport Events"),
              ],
            ),
          )
        : Container(
